...
var (
	...
	// body.
	//
	// ErrBodyNotAllowed 当调用 ResponseWriter.Write，而 HTTP method 或 response code 不允许写入 body 时返回错误。
	...
	// effects.
	//
	// ErrHijacked 当调用 ResponseWriter.Write，而底层连接通过 Hijacker 接口被劫持时返回错误。
	// 零字节被写入到被劫持的连接中，并返回 ErrHijacked 且没有任何其他的副作用。
	...
	// declared.
	//
	// ErrContentLength 当调用 ResponseWriter.Write，response header 中设置了 Content-Length 的大小却尝试写入更多的字节时返回错误。
	...
	//
	// 弃用：ErrWriteAfterFlush
	// net/http 没有任何方法返回此错误。
	// 不应该与此变量进行比较。
...
type Server struct {
	...
	// 用于执行的 handler，如果为空会执行 http.DefaultServeMux
	...
	// instead.
	//
	// TLSConfig 可选择地提供 TLS 配置给 ServeTLS 和 ListenAndServeTLS 使用。
	// IMP: 注意 ServeTLS 和 ListenAndServeTLS 会进行拷贝，所以无法使用
	// tls.Config.SetSessionTicketKeys 等方法修改配置要使用 SetSessionTicketKeys，
	// 请使用 TLS Listener 启动 Server.Serve。
	...
	//
	// ReadTimeout 读取包含 body 的完整 request 的最大时间。
	//
	// 因为 ReadTimeout 不允许 Handlers 根据每个请求的 body 可接受的截止时间或上传速率
	// 对每个请求作出决策，因此大多数用户更愿意使用 ReadHeaderTimeout。使用这两个参数都是
	// 有效的。
	...
	// 连接的读取截止时间在读取请求头后被重置，并且 Handler 可以决定哪一方面对主体来说太慢。
	...
	//
	// WriteTimeout 是写操作响应前对最大持续时间。每个新的请求头被读取时重置它。
	// 同 ReadTimeout 一样，它也不允许基于每个请求做出决策。
	...
	//
	// IdleTimeout 是在开启 keep-alives 时，等待下一个请求对最大空闲时间。
	// 如果 IdleTimeout 为 0，将使用 ReadTimeout 的值。
	// 如果两者都为 0，将使用 ReadHeaderTimeout 的值。
	...
	//
	// MaxHeaderBytes 控制服务器将读取解析请求头 keys 和 values（包括请求行）的最大字节数。
	// 它并不限制请求主体对大小。
	// 如果为 0，将会使用 DefaultMaxHeaderBytes。
	MaxHeaderBytes int
	...
	//
	// ConnState 指定一个可选的回调函数，在客户端连接状态改变时调用。
	// 详细信息请查看 ConnState 类型及其相关常量。
	...
	//
	// ErrorLog 为接受连接时错误、处理程序的意外行为、底层文件系统错误指定可选的日志记录程序。
	// 如果为 nil，日志将通过 log 包的标准日志记录程序完成记录。
	ErrorLog *log.Logger

	// 以原子方式访问
	disableKeepAlives int32 // accessed atomically.
	// 以原子方式访问（非零值表示我们正处于 Shutdown 状态）
	inShutdown int32 // accessed atomically (non-zero means we're in Shutdown)
	// 保护 setupHTTP2_* 初始化
	nextProtoOnce sync.Once // guards setupHTTP2_* init
	// 使用 http2ConfigureServer 返回的 http2.ConfigureServer 结果
...
// ，才开启 HTTP/2 的支持。
//
// Serve 永远返回一个非空的错误并关闭 Listener l。
// 在执行 Shutdown 或 Close 后，返回错误 ErrServerClosed。
